...
public class TargetSelectionAddArgumentType implements ArgumentType<TargetFilter> {
    ...
    private TargetSelectionAddArgumentType(TargetSelectionProperty property) {
        this.property = property;
    }

    public static TargetSelectionAddArgumentType addTargetSelection(TargetSelectionProperty property) {
        return new TargetSelectionAddArgumentType(property);
    }
    ...
    public TargetFilter parse(StringReader reader) throws CommandSyntaxException {
        ...
        if (this.property.getValue().contains(targetFilter))
            throw TARGET_FILTER_ALREADY_PRESENT.createWithContext(reader, s);
        return targetFilter;
    }
